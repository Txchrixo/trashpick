...
class AdminPickerHistoryController extends GetxController {
  final FirestoreService _firestoreService = FirestoreService();

  final Rx<AppUser?> picker = Rx<AppUser?>(null);
  ...
  void onInit() {
    super.onInit();
    picker.value = Get.arguments as AppUser?;
  ...
  void _loadPickerRequests() {
    isLoading.value = true;

    // Fetch requests WITHOUT orderBy to avoid composite index requirement
    _firestoreService.listenToPickerTrashReports(picker.value!.id).listen(
      (requests) {
  ...
  List<TrashReport> get activeRequests {
    return allRequests
        .where((request) => request.status == TrashStatus.inTransit)
        .toList();
  }

  // Completed requests with filters applied
  List<TrashReport> get filteredCompletedRequests {
    var completed = allRequests
        .where((request) => request.status == TrashStatus.completed)
        .toList();
